...
- **Rails 파일 생성은 generator 우선**: 모델, 마이그레이션, 컨트롤러, 테스트 등 Rails가 생성할 수 있는 파일은 직접 새 파일로 만들지 말고 `bin/rails generate ...`로 생성한 뒤 필요한 내용을 수정한다.
- Engine 내부 파일을 루트 generator로 생성해야 하는 경우, generator 산출물을 확인한 뒤 엔진 경로로 이동하고 불필요한 루트 산출물은 제거한다.
